...
(function () {
  ...
  function renderCart() {
    ...
    const swishBtn = document.getElementById('swish-checkout-button');
    ...
    itemsEl.innerHTML = entries
      .map(({ product, qty }) => {
        ...
        const art = product.image_url
          ...
          : productIcon(product.icon, product.accent);
        return `
        <div class="cart-item">
          <div class="cart-item-art">${art}</div>
  ...
  async function goToCheckout() {
    ...
    try {
      ...
      checkoutBtn.disabled = false;
      checkoutBtn.textContent = 'Betala med kort';
    }
  }

  // ---------- Swish ----------
  function openSwish() {
    document.getElementById('swish-form-step').hidden = false;
    document.getElementById('swish-pay-step').hidden = true;
    document.getElementById('swish-done-step').hidden = true;
    document.getElementById('swish-form-error').style.display = 'none';
    document.getElementById('swish-modal').classList.add('open');
    document.getElementById('swish-overlay').classList.add('open');
  }

  function closeSwish() {
    document.getElementById('swish-modal').classList.remove('open');
    document.getElementById('swish-overlay').classList.remove('open');
  }

  let currentSwishOrderId = null;

  async function submitSwishOrder() {
    const errorEl = document.getElementById('swish-form-error');
    errorEl.style.display = 'none';

    const customer = {
      name: document.getElementById('swish-name').value.trim(),
      email: document.getElementById('swish-email').value.trim(),
      address: document.getElementById('swish-address').value.trim(),
      postalCode: document.getElementById('swish-postal').value.trim(),
      city: document.getElementById('swish-city').value.trim(),
    };

    if (!customer.name || !customer.email || !customer.address) {
      errorEl.textContent = 'Namn, e-post och adress krävs.';
      errorEl.style.display = 'block';
      return;
    }

    const submitBtn = document.getElementById('swish-submit-button');
    submitBtn.disabled = true;
    submitBtn.textContent = 'Skapar order…';

    const items = Object.entries(cart).map(([id, quantity]) => ({ id, quantity }));

    try {
      const res = await fetch('/api/swish-order', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ items, customer }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Kunde inte skapa ordern.');

      currentSwishOrderId = data.orderId;

      const qrEl = document.getElementById('swish-qr');
      qrEl.innerHTML = '';
      // eslint-disable-next-line no-undef
      new QRCode(qrEl, { text: data.swishLink, width: 220, height: 220, correctLevel: QRCode.CorrectLevel.M });

      document.getElementById('swish-order-number-text').textContent = data.orderNumber;
      document.getElementById('swish-amount-text').textContent = `${data.amountKr.toLocaleString('sv-SE')} kr`;
      document.getElementById('swish-open-link').href = data.swishLink;

      document.getElementById('swish-form-step').hidden = true;
      document.getElementById('swish-pay-step').hidden = false;
    } catch (err) {
      console.error(err);
      ...
      submitBtn.textContent = 'Visa Swish-betalning';
    }
  }

  async function confirmSwishPaid() {
    const btn = document.getElementById('swish-paid-button');
    btn.disabled = true;
    btn.textContent = 'Sparar…';

    try {
      if (currentSwishOrderId) {
        await fetch('/api/swish-order', {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ orderId: currentSwishOrderId }),
        });
      }
    } catch (err) {
      console.error(err);
      // Fortsätt ändå — det här är bara en flagga, inte en betalningsbekräftelse.
    }
    ...
    document.getElementById('swish-pay-step').hidden = true;
    document.getElementById('swish-done-step').hidden = false;
    btn.disabled = false;
    btn.textContent = 'Jag har betalat';
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('swish-checkout-button').addEventListener('click', openSwish);
    document.getElementById('swish-close').addEventListener('click', closeSwish);
    document.getElementById('swish-overlay').addEventListener('click', closeSwish);
    document.getElementById('swish-submit-button').addEventListener('click', submitSwishOrder);
    document.getElementById('swish-paid-button').addEventListener('click', confirmSwishPaid);
    document.getElementById('swish-close-done-button').addEventListener('click', closeSwish);
