...
create table if not exists products (
  ...
  icon text not null default 'ball',       -- vilken inbyggd ikon som ritas om ingen bild finns
  accent text not null default '#1B4332',  -- hexfärg som tonar ikonen
  image_url text,                          -- valfri produktbild (annars visas ikonen ovan)
...
-- ---------- Lagring för produktbilder ----------
-- Skapar en publik bucket för produktbilder. Uppladdning sker bara via
-- /api/admin/upload-image (kräver admin-lösenord) med service role-nyckeln,
-- så det behövs ingen publik skriv-policy här — bara publik läsning.
...
create table if not exists orders (
  ...
  payment_method text not null default 'stripe', -- 'stripe' eller 'swish'
  customer_email text,
  customer_name text,
  shipping_address text,
  amount_total_cents integer not null,
  status text not null default 'pending', -- pending -> paid -> fulfilled (du bestämmer flödet)
  customer_marked_paid boolean not null default false, -- kunden klickade "Jag har betalat" (Swish)
  created_at timestamptz not null default now()
);

-- Säkerställer att kolumnerna finns även om orders-tabellen skapades med en
-- äldre version av detta schema (körs säkert flera gånger).
...
alter table orders add column if not exists customer_marked_paid boolean not null default false;
