...
Visit `https://YOUR-SITE.pages.dev/admin` to manage products and review
orders (both Swish and card). It's not linked from anywhere on the public
site, and it's blocked from search engines via `robots.txt`, but the URL
itself isn't a secret — anyone who guesses or finds it will hit a password
prompt.
...
Add `SWISH_NUMBER` (your number, digits only, e.g. `1234567890`) as an
environment variable in Cloudflare Pages alongside the others.

Confirmation is manual — always double-check your Swish app before
shipping. If your business later gets a real Swish Handel agreement with a
bank, that would allow automatic payment confirmation, but that's a
separate, more involved integration than what's here.
